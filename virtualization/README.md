...
VirtualBox is installed via `pacman` on Arch Linux (`virtualbox`, `virtualbox-host-dkms` and `linux-headers`). `virtualbox-host-dkms` is used instead of the precompiled `virtualbox-host-modules-arch`, since that package only targets the stock `linux` kernel and is not resolvable/compatible on installs running a different kernel; DKMS builds against whatever kernel headers are installed instead. Vagrant has no official Arch Linux package (AUR-only) and is installed via `yay` instead - a deliberate exception to this collection's usual no-AUR-helper policy, the same treatment as `visual-studio-code-bin` in the `vscode` role. The role temporarily grants `default_user` passwordless `sudo` for `pacman` while `yay` runs, then revokes it again, since `yay` internally shells out to `sudo pacman -U ...` to install the finished package and that nested `sudo` call cannot prompt for a password under Ansible.
