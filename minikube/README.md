# Role: Minikube

The role installs and configures Minikube, a tool for running Kubernetes clusters locally. Additionally it installs Helm, a package manager for Kubernetes, and all dependencies expect for the virtualization provider.

Minikube automatically selects the virtualization provider based on the host system. Just make sure Docker is installed before starting minikube.

The role is intended to be used on Ubuntu Desktop machines.

## Expected Variables

| Variable             | Description |
|----------------------|-------------|
| `{{ ansible_user }}` | The user to install and configure for (typically the logged-in user). |
