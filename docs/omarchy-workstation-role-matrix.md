...
Arch support only ever uses `community.general.pacman` against the official Arch repositories. Tools that exist only in the AUR are never installed automatically by these roles - see the "AUR-only / excluded" notes below. The one deliberate exception is `vscode` (see its row and note below).
