...
Arch support only ever uses `community.general.pacman` against the official Arch repositories. Tools that exist only in the AUR are never installed automatically by these roles - see the "AUR-only / excluded" notes below.
...
| `virtualization` (Vagrant)        | Vagrant                    | yes    | no   | no                       | no                 | no (AUR-only)           | exclude  |
| `vscode`                          | VS Code                    | yes (MS build, snap) | yes (Code - OSS) | no | yes      | partial                 | include (different build, documented in role README) |
...
- **AUR-only tools** still excluded from Arch automation for the remaining case: Vagrant. It requires an AUR helper (`yay`/`paru`), which this collection deliberately does not depend on. Omarchy ships `yay` itself and users can install it manually if desired.
